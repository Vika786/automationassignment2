resource "azurerm_public_ip" "lbpubip" {
  ...
  domain_name_label  = "assignment2-2096"
}


resource "azurerm_lb" "assignment2" {
  name                = "lb-assignment1-2096"
  location            = var.location
  resource_group_name = var.rg
  tags                  = local.common_tags
  frontend_ip_configuration {
    name                 = "PublicIPAddress-2096"
...
resource "azurerm_network_interface_backend_address_pool_association" "lb_pool_association1" {
  ...
  ip_configuration_name   = "linux5-centos-vm-ipconfig1"
...
resource "azurerm_network_interface_backend_address_pool_association" "lb_pool_association2" {
  ...
  ip_configuration_name   = "linux5-centos-vm1-ipconfig1"
  backend_address_pool_id = azurerm_lb_backend_address_pool.bpepool.id
}
...
resource "azurerm_lb_rule" "lb_rule" {
  ...
  frontend_port                  = 80
  backend_port                   = 80
  frontend_ip_configuration_name = "PublicIPAddress-2096"
