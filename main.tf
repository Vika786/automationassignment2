...
module "linux" {
  ...
  linux1_name                 = {linux5-centos-vm = "Standard_B1s"
                                linux5-centos-vm1 = "Standard_B1s"              }
  linux_avs  = "linux-avs"
...
resource "null_resource" "ansible-deploy" {
  ...
  provisioner "local-exec" {
    command = "export ANSIBLE_HOST_KEY_CHECKING=False && ansible-playbook --private-key /home/automation/.ssh/id_rsa -i hosts groupX-playbook.yml"
  }
}
